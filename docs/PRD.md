...
- **详细人设（detailed_persona）**：Agent 自己看到的完整角色设定，直接注入 Prompt 第 2 层。决定它的性格、语气、知识、行为方式。这是它的"自我认知"。
- **简易人设（simplified_persona）**：其它 Agent 在与之互动时，在 Prompt 第 3 层看到的关于它的简介。例如：
...
- **辅助人设字段**（`personality`、`scenario`、`example_messages`、`first_message`）：
  ...
  - **未来用途**：用于"Agent 自我迭代升级人设"功能（P2）。Agent 可以基于这些素材和自我对话历史，自动优化自己的 `detailed_persona`。
...
**编辑 Agent 的生效规则**：
- 用户修改 Agent 的任何字段（人设、模型参数等）后，点击"保存"立即生效。
- **所有现有会话**中涉及该 Agent 的 Prompt 拼接，在**下一次触发时自动使用最新配置**。
...
7. **Agent 也可以通过 `create_private_chat` Tool 自主发起与其它 Agent 的私聊**（见 5.4.3）。Agent 发起的私聊同样会建立好友关系，并实时显示在用户的会话列表中。
...
5. **再次创建会话**：如果用户删除私聊后重新与同个 Agent 创建私聊，系统**创建全新的 session**（多线程独立上下文），不会恢复旧 session。
...
系统为每个 Agent 提供以下 Function Calling 工具：
...
**2. `create_private_chat` Tool（Agent 自主发起私聊）**

| 参数 | 类型 | 说明 |
|-----|------|------|
| `target_agent_id` | string | 目标 Agent 的唯一 ID |
| `first_message` | string | 私聊的第一条消息内容（可选） |

**`create_private_chat` 调用行为**：
- Agent 可以在群聊中或通过全局历史获知其它 Agent 的存在，并主动发起私聊。
- 系统收到 `create_private_chat` tool_call 后：
  1. 检查两 Agent 之间是否已存在私聊会话。
  2. 若不存在，创建新的私聊会话（session_type = 'private'）。
  3. 在 `friendships` 表中建立双方的好友关系（若已存在则跳过）。
  4. 如果提供了 `first_message`，将该消息写入新会话，触发对方 Agent。
  5. **不将执行结果返回给调用 Agent**。
...
```
1. 系统提示词（全局固定，所有 Agent 相同，代码中硬编码）
...
- **系统提示词（第1层）**：所有 Agent 共享，用于设定基础行为规则（如"你是一个 IM 聊天参与者"）。**必须包含会话选择指令**："你可以同时参与多个私聊和群聊。在回复时，请根据上下文判断应该回复哪个会话。如果需要回复多个会话，可以多次调用 send_message 工具。"
- **自身人设（第2层）**：Agent 自己的详细人设，不可被其它因素覆盖。
- **参与者简介（第3层）**：动态根据"好友关系 + 群友关系"筛选后拼接，使用它们的**简易人设**。这让 Agent 知道"我在和谁聊天"，同时避免无关角色干扰上下文。
- **历史消息（第4层）**：从该 Agent 的可见消息历史中按私聊/群聊分组提取，保持时间顺序。包含该 Agent 参与的全部会话（所有私聊 + 所有群聊）。
- **最新消息（第5层）**：单独高亮触发本次调用的消息，并**标注来源会话**，让 Agent 明确知道"这条消息来自哪个会话"以及"我这次应该优先回应哪个会话"。Agent 也可以基于其它会话的上下文决定回复另一个会话。
...
- **Agent-Agent 好友**：当两个 Agent 被创建为 1对1 私聊会话后（或通过 `create_private_chat` Tool 自主发起），双方自动标记为"好友"。好友关系是双向的、持久化的。删除私聊会话**不会**自动删除好友关系（类似 IM 中删会话不等于删好友）。未来可扩展手动添加/删除好友功能（P2）。
- **用户-Agent 关系**：**所有 Agent 默认将用户视为好友**。在 Prompt 的参与者简介中，用户统一标注为：
  ```
  - {用户昵称}（真实用户）：正在与你聊天的真实用户。
  ```
  - P2 阶段启用用户人设后，上述标注替换为用户选择的当前人设信息。
  - 不需要在 `friendships` 表中维护用户-Agent 关系（用户是隐式全局好友）。
...
- 用户也可点击"重置计数"按钮手动恢复。点击后：
  1. 计数器重置为 0。
  2. 系统以当前会话的完整聊天记录作为上下文，**立即触发该会话中所有相关 Agent 一次**（私聊触发对方 Agent，群聊触发所有其它 Agent）。
- 用户还可在设置中临时关闭上限或调高数值。
...
- **用户 → Agent**：用户发送消息后，将消息加入该 Agent 的积压队列，按全局最小间隔规则触发。
  - **用户连续发送多条消息的场景**：第一条消息到达时若间隔满足，**立即触发** Agent（此时 pending_queue 中只有第一条）。Agent 回复后，后续消息继续累积在 pending_queue 中，等待间隔到期后批量处理。
  - **消息顺序保持**：前端严格按 `created_at` 时间戳展示消息。例如：用户1 → AI回复1 → 用户2 → 用户3 → 用户4 → 用户5 → AI回复2（批量）。
- **Agent → 用户**：Agent 发送消息后，接收方是真实用户，不再触发自动调用。
- **Agent A → Agent B（两个 Agent 的私聊）**：Agent-Agent 私聊可由用户手动创建，也可由 Agent 通过 `create_private_chat` Tool 自主发起。Agent A 发送消息后，触发 Agent B（加入积压队列，按间隔触发）。Agent B 回复后，触发 Agent A。理论上可无限循环，但受以下机制约束：
...
2. 如果多个 Agent 同时决定回复：
    - 每个 Agent 的回复在**生成完成后立即展示**在群聊中（来一个展示一个），按生成完成的时间排序。
...
- **Agent 消息**：靠左展示，使用白色/灰色气泡，显示 Agent 头像和名称
  - **不采用流式输出**：Agent 消息在生成完成后**一次性完整展示**，不逐字/逐句打字机式输出。
  - **私聊"正在输入"**：在 1对1 私聊窗口中，Agent 生成期间可显示"正在输入..."提示（低优先级，P1 实现）。
