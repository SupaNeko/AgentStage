...
<div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 modal-overlay"
    ...
    onclick={(e) => { if (mouseDownOnOverlay && e.target === e.currentTarget) open = false; mouseDownOnOverlay = false; }}
    role="dialog" aria-modal="true">
    <div class="bg-surface rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto modal-card" onmousedown={() => mouseDownOnOverlay = false} onclick={(e) => e.stopPropagation()}>
