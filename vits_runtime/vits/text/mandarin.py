...
jieba.set_dictionary(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'jieba', 'dict.txt'))
jieba.initialize()
...
def VITS_PinYin_model():
    ...
    if not os.path.isdir(bert_dir):
        raise RuntimeError(
            "bert_chinese_cleaners requires the bert prosody model directory (vits/text/../bert), "
            "which is not bundled with this runtime."
        )
    tts_front = VITS_PinYin(bert_dir, device)
    return tts_front
